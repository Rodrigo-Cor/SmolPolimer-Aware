<template>
  <div class="container-fluid" style="background-color: #effffb">
    ...
    <section class="text-justify-custom my-2">
      ...
      <button @click="() => goReferences('filtracion0')" type="button" class="btn btn-link btn-sm p-0 m-0">
        [1].
      </button>
    </section>
  ...
  Unsplash:
    ...
    <section class="text-justify-custom my-2">
      Al seleccionar "Simulación con valores por defecto", se introducen al simulador 29 mg de microplásticos que
      durante 9 días se estarán filtrando, al mismo tiempo que se indican 9 mg de residuos microplásticos para el cálculo.
      Si por otro se escoge el formulario, se limita hasta 40 mg de microplásticos, 20 mg de residuos y un máximo de 24
      días; se recomienda introducir una cantidad considerable de días, ya que dentro de estas restricciones el simulador
      muestra el comportamiento deseado.
    </section>
    ...
    <FiltracionPDF :svgData="svgData" v-if="onFilterValues.length > 0 && releasedValues.length > 0 && svgData" />
    ...
    <article class="mt-2">
      ...
      <SectionReferences :view="'filtracion'" :references="[
        {
          authors: [
